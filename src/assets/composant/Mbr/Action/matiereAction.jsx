...
const MatiereactionManagerWithTable = () => {
  ...
  const handleAddRow = async (id_mat) => {
    try {
      const newRowId = tableData[id_mat].lignes.length > 0 
        ? Math.max(...tableData[id_mat].lignes.map(l => l.row_id)) + 1 
        : 1;
      ...
      setTableData(prev => ({
        ...
        [id_mat]: {
          ...prev[id_mat],
          lignes: [...prev[id_mat].lignes, newRow]
        }
      }));

    } catch (err) {
      setError(err.response?.data?.error || "Erreur lors de l'ajout d'une ligne");
    }
  };
              ...
              {activeTable === matiere.id_mat && tableData[matiere.id_mat] && (
                <div className="p-6 border-t border-gray-100 animate-fadeIn">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                      onClick={() => handleAddRow(matiere.id_mat)}
...
export default MatiereactionManagerWithTable;