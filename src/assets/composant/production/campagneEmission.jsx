...
const CampagneEmission = () => {
  ...
  const fetchCampagnes = async () => {
    try {
      const res = await axios.get("http://localhost:3000/api/campagne/affichebystatut/Emission");
  ...
  const handleLancer = async (camp) => {
    try {
      await axios.put(`http://localhost:3000/api/campagne/commencer/campBR/${camp.id_camp}`);
      fetchCampagnes();
      alert(`Campagne "${camp.nom_fab}" lancée`);
      navigate(`/PROD/campagneProduit`);
    } catch (err) {
    ...
    <div className="bg-gray-100 min-h-screen p-6">
      ...
      {campagnes.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-xl text-gray-600">Aucune campagne reçue</p>
        </div>
      ) : (
...
export default CampagneEmission;